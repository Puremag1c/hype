...
0. **TASK BUDGET:** Создай **не больше задач чем указано в TASK BUDGET** (см. контекст ниже). Тесты пишутся ВНУТРИ задач executors — не создавай отдельные задачи на тесты для каждого модуля.
