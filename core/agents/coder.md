...
9. **НИКОГДА не создавай новые задачи (`bd create`)** — только Architect может создавать задачи. Исключение: rebase conflict escalation (см. секцию "6. Rebase")
10. **НИКОГДА не модифицируй `.gitignore`** — особенно не добавляй `!` исключения для `.hype/`. Структура gitignore управляется `hype init`/`hype upgrade`
